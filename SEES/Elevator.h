...
SC_MODULE(Elevator){
    ...
    sc_in<bool>      closeDoor;          // control sends the elevator the signal to continue closing the door
    ...
    SC_CTOR(Elevator){
        ...
        SC_THREAD(drive);
        sensitive << nextMode << nextTarget << barrier[0] << barrier[1] << barrier[2] << stopHere << closeDoor;
